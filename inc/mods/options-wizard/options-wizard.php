	...
	class Presscore_Modules_OptionsWizardModule {
		...
		public static function print_wizard_mode_selector_action() {
			...
			<div class="of-info-block hide-if-js">
				<p><?php echo _x( 'Мастер Опций темы работает в двух режимах: это позволяет "Настройка существующего дизайна" или  "Начните с чистого листа".', 'theme-options', 'the7mk2' ); ?></p>
				<p><?php printf( _x( '<strong>Внимание!</strong> Если вы решите  "Начать с чистого листа", Мастер автоматически рассчитает и перезапишет большинство ваших настроек! Вы можете использовать <a href="%s">Экспорт/Импорт Параметров</a> интерфейс для резервного копирования текущего состояния параметров темы перед созданием.', 'theme-options', 'the7mk2' ), admin_url( 'admin.php?page=of-importexport-menu' ) ); ?></p>
				<p>
					<a class="button-secondary" href="<?php echo esc_url( add_query_arg( array( 'page' => self::$options_page_id, 'wizard_mode' => 'from_scratch' ), admin_url( 'admin.php' ) ) ); ?>" onclick="return confirm( '<?php print esc_js( _x( 'Внимание! По “Начиная с чистого листа", Вам сбросите большинство параметров внешнего вида вашего сайта! Вы хотите продолжить?', 'theme-options', 'the7mk2' ) ); ?>' );"><?php echo esc_html( _x( 'Начните с чистого листа', 'theme-options', 'the7mk2' ) ); ?></a>
					<input class="button-primary" value="<?php echo esc_attr( _x( 'Настройка существующего дизайна', 'theme-options', 'the7mk2' ) ); ?>" type="button">
				</p>
			</div>
